...
interface Message {
  ...
  role: "user" | "assistant";
  content: string;
  timestamp: string; // store as ISO for serialization safety
}
...
export default function ChatbotPage() {
  ...
  useEffect(() => {
    // whenever messages change (new message, assistant finished typing), scroll to bottom
    scrollToBottom("auto");
  ...
  const typeMessage = (text: string) => {
    ...
    // a comfortable speed on mobile and desktop (12-18ms) — allow speedy typing, but readable
    const intervalMs = Math.max(8, Math.round(1000 / 120));
    ...
    const interval = window.setInterval(() => {
      if (index < text.length) {
        ...
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), role: "assistant", content: text, timestamp: new Date().toISOString() },
        ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const historyToSend = [...messages, userMessage].map((msg) => ({ role: msg.role, content: msg.content }));

      const response = await fetch(`${API_BASE_URL}/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ personaId, message: userMessage.content, conversationHistory: historyToSend }),
      });
  ...
  return (
    <div className="chat-wrapper">
      <div className="chat-container">
        ...
        <main className="messages-container" role="log" aria-live="polite">
          <div className="messages-wrapper">
            {isChatEnded && <div className="end-banner">Conversation ended.</div>}

            {messages.map((msg) => (
              <article
                key={msg.id}
                className={`message ${msg.role === "user" ? "message-user" : "message-assistant"}`}
                aria-roledescription={msg.role}
              >
                ...
                <div className="message-content">
                  <div className="message-header">
                    <span className="message-role">{msg.role === "user" ? "You" : persona?.name}</span>
            ...
            {isTyping && !isChatEnded && (
              <div className="message message-assistant" aria-hidden>
                <div className="message-avatar">
                  <div className="avatar-assistant">
                    <Zap size={18} />
                  </div>
        ...
        {!isChatEnded && (
          <div className="input-section" role="region" aria-label="Message input">
            <div className="input-container">
              <div className="input-wrapper">
                ...
                <div className="controls">
                  <button onClick={handleEndChat} className="end-chat-button" title="End Conversation">
                    ✕
                  </button>

                  <button onClick={handleSubmit} disabled={!input.trim() || isLoading} className="send-button" aria-label="Send message">
                    <Send size={20} />
                  </button>
                </div>
              </div>
